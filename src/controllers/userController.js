...
const signUp = async (req, res) => {
    ...
    if (!email || !password || !nombre || !genero) {
    ...
    try {
        ...
        // Guardar detalles adicionales en la tabla usuarios
        console.log(data.user);
        const  uuid  = data.user.id;
        console.log(uuid);
        const { error: insertError } = await supabaseClient.from('usuarios').insert([
            { uuid, nombre, genero, created: new Date().toISOString() },
        ]);
...
const verify = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ user: { id: user.user.id, nombre: userData.nombre } });
    } catch (err) {
